 ...
 *                                                                           
 * @author Fawzi Emad, Ethan Dessner             
 */
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		boolean fishInSpace = false;
		boolean plantInSpace = false;
		for(Fish fishCheck : fish) {
			if(fishCheck.getRow() == r && fishCheck.getCol() == c) {
				fishInSpace = true;
				break;
			}
		}
		for(Plant plantCheck : plants) {
			if(plantCheck.getRow() == r && plantCheck.getCol() == c) {
				plantInSpace = true;
				break;
			}
		}
		if(fishInSpace == true || plantInSpace == true || landscape[r][c] == ROCK)
			return false;
		else
			return true;
	}
	...
	public Model(Model other) {
		...
		for(int i = 0; i < other.landscape.length; i++) {
			landscape[i] = other.landscape[i];
		}
		fish = new ArrayList<Fish>();
		plants = new ArrayList<Plant>();
		for(Fish fishCheck : other.fish) {
			this.fish.add(new Fish(fishCheck));
		}
		for(Plant plantCheck : other.plants) {
			this.plants.add(new Plant(plantCheck));
		}
	}
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		if(landscape[f.getRow() + 1][f.getCol()] == ROCK && //RIGHT
				landscape[f.getRow() - 1][f.getCol()] == ROCK && //LEFT
				landscape[f.getRow()][f.getCol() + 1] == ROCK && //UP
				landscape[f.getRow()][f.getCol() - 1] == ROCK) { //DOWN
			return true;
		}
		return false;
	}
	...
	public void turnFish() {
		for(Fish currentFish : fish) {
			if(currentFish.isAlive() && fishIsSurroundedByRocks(currentFish) == false) {
				if(currentFish.getDirection() != Fish.UP &&
						currentFish.getDirection() != Fish.DOWN &&
						currentFish.getDirection() != Fish.LEFT &&
						currentFish.getDirection() != Fish.RIGHT) {
					throw new IllegalFishDirectionException(currentFish.getDirection());
				}
				int rowMove = currentFish.getRow();
				int colMove = currentFish.getCol();
				if(currentFish.getDirection() == Fish.UP) {
					rowMove--;
				}
				if(currentFish.getDirection() == Fish.DOWN) {
					rowMove++;
				}
				if(currentFish.getDirection() == Fish.LEFT) {
					colMove--;
				}
				if(currentFish.getDirection() == Fish.RIGHT) {
					colMove++;
				}

				while(landscape[rowMove][colMove] == ROCK) {
					currentFish.setRandomDirection();
					rowMove = currentFish.getRow();
					colMove = currentFish.getCol();
					if(currentFish.getDirection() == Fish.UP) {
						rowMove--;
					}
					if(currentFish.getDirection() == Fish.DOWN) {
	...
	public void moveFish() {
		for(Fish fishCheck : fish) {
			if(fishCheck.isAlive()) {
				if(fishIsSurroundedByRocks(fishCheck) == false) {
					fishCheck.move();
					for(Plant plantCheck : plants) {
						//plantCheck.isAlive() &&
						if(plantCheck.isAlive() && plantCheck.getRow() == fishCheck.getRow() && plantCheck.getCol() == fishCheck.getCol()) {
							fishEatPlant(fishCheck, plantCheck);
						}
					}
					for(Fish otherFish : fish) {
						if(otherFish.isAlive()) {
							if(fishCheck != otherFish && otherFish.getRow() == fishCheck.getRow() && otherFish.getCol() == fishCheck.getCol())
								fishCheck.fight(otherFish);
						}
					}
				}
			}
	...
	public void addPlant(Plant p) {
		...
		for(int i = 0; i < plants.size(); i++) {
			if(plants.get(i).getRow() == p.getRow() && plants.get(i).getCol() == p.getCol()) {
	...
	public void addFish(Fish f){
		...
		for(int i = 0; i < fish.size(); i++) {	
			if(fish.get(i).getRow() == f.getRow() && fish.get(i).getCol() == f.getCol()) {
